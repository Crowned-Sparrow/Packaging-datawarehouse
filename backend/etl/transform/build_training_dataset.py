...
def _empty_feature_frame() -> pl.DataFrame:
    return pl.DataFrame(
        schema={
            ...
            "machine_id": pl.Int64,
            "total_runs": pl.Int64,
            "total_production_minutes": pl.Float64,
            "breakdown_count": pl.Int64,
            "downtime_minutes": pl.Float64,
        }
    )
...
def build_training_dataset_df(
    production_frame: pl.DataFrame,
    breakdown_frame: pl.DataFrame,
) -> pl.DataFrame:
    production_features = _empty_feature_frame()
    if production_frame.height > 0:
        production_features = (
            production_frame.with_columns(pl.col("start_time").dt.date().alias("feature_date"))
            .group_by(["feature_date", "machine_id"])
            .agg(
